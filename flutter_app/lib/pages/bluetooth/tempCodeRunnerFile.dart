...
class _MatchedDevicePageState extends State<MatchedDevicePage> {
      ...
      _receiveBatteryStateListener = char.onValueReceived.listen((value) {
        if (value.isNotEmpty && mounted) {
          try {
            ...
            Map<String, dynamic> data = jsonDecode(rawString);
            
           
            double pressure = (data['pressure'] ?? 0).toDouble();

            setState(() {
              deviceBattery = (data['battery'] ?? 0).toInt();
            });
          } catch (e) {
            debugPrint("แกะข้อมูล JSON ไม่สำเร็จ: $e");
          }
        }
      });
    }
  ...
  void handleDiscoverServices() async {
    ...
    // เปลี่ยนมาใช้ loop เพื่อกัน Error "Bad state: No element"
      ...
      if (sUuid.contains(suctionServiceUuid.toUpperCase())) {
        for (var characteristic in service.characteristics) {
          ...
          if (uuid.contains(sensorUuid.toUpperCase())) {
            ...
            // เมื่อเจอตัวเซนเซอร์ ให้เรียกฟังก์ชันรับค่า (ซึ่งเราจะแก้ให้รับ JSON ได้)
